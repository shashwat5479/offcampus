...
export function ConfessionComposer() {
  ...
  return (
    <div className="rounded-xl2 border border-line bg-paper p-4">
      <textarea
        value={body} onChange={(e) => setBody(e.target.value)} rows={3} maxLength={1000}
      ...
      <div className="mt-2 flex items-center justify-between">
        <label className="flex items-center gap-2 text-xs text-subtle">
          <input type="checkbox" checked={isPublic} onChange={(e) => setIsPublic(e.target.checked)} className="h-3.5 w-3.5 accent-[rgb(var(--c-accent))]" />
          {isPublic ? "All colleges" : "My college only"}
        </label>
        <button onClick={post} disabled={busy || body.trim().length < 3}
...
export function ConfessionItem({ c }) {
  ...
  async function up() {
    const prevS = score, prevD = dir;
    const next = dir === 1 ? 0 : 1;
    setDir(next); setScore(score + (next - dir));
    try {
    ...
    } catch { setDir(prevD); setScore(prevS); }
  }
  ...
  return (
    <div className="rounded-xl2 border border-line bg-paper p-4">
      <div className="mb-1 flex items-center gap-2 text-[11px] text-faint">
        ...
        {c.isPublic && <span className="rounded bg-accent/12 px-1.5 py-0.5 font-medium text-accent">All colleges</span>}
        <span>· {timeAgo(c.createdAt)}</span>
      </div>
      <p className="whitespace-pre-wrap text-sm text-ink">{c.body}</p>
      <button onClick={up} className={`mt-2 flex items-center gap-1.5 rounded-full px-2 py-1 text-xs font-medium ${dir === 1 ? "text-up" : "text-subtle hover:text-up"}`}>
        <svg width="14" height="14" viewBox="0 0 24 24" fill={dir === 1 ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2"><path d="M12 5l7 8h-4v6H9v-6H5z"/></svg>
        {score}
      </button>
    </div>
  );
