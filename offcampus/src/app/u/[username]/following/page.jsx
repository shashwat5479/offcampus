...
export default async function FollowingPage({ params }) {
  ...
  const rows = await prisma.follow.findMany({
    ...
    include: { following: { include: { college: true } } },
  });
  const people = rows.map((r) => r.following);
    ...
    <div className="mx-auto max-w-feed">
      <div className="mb-3 flex items-center gap-3">
        ...
        <h1 className="text-lg font-semibold tracking-tight">
          Following <span className="text-subtle">· @{user.username}</span>
        </h1>
      </div>

      <div className="rounded-xl2 border border-line bg-paper px-4">
        {people.length === 0 ? (
          <p className="py-8 text-center text-sm text-subtle">Not following anyone yet.</p>
        ) : (
