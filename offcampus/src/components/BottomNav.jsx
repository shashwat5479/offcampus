  ...
  const slot = (href, label, icon) => {
    ...
    return (
      <Link key={href} href={href} aria-label={label}
        className={`flex flex-col items-center gap-0.5 rounded-full px-3.5 py-1.5 transition-colors ${active ? "bg-accent/15 text-accent" : "text-subtle hover:text-ink"}`}>
        {icon}
  ...
  return (
    <nav className="fixed bottom-4 left-1/2 z-50 -translate-x-1/2">
      <div className="flex items-center gap-1 rounded-full border border-line bg-paper/95 px-2 py-1.5 shadow-2xl backdrop-blur">
        {slot("/", "Home", I.home)}
        {slot("/radar", "Radar", I.radar)}
        <Link href="/submit" aria-label="Create" className="flex flex-col items-center gap-0.5 px-2 py-1">
          <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-accent text-white shadow-glow transition-transform active:scale-90">{I.plus}</span>
          <span className="text-[10px] font-medium text-subtle">Create</span>
        </Link>
        {slot("/opportunities", "Opportunities", I.opportunities)}
        <Link href={`/u/${user.username}`} aria-label="Profile"
          className={`flex flex-col items-center gap-0.5 rounded-full px-3.5 py-1.5 transition-colors ${profileActive ? "text-accent" : "text-subtle hover:text-ink"}`}>
